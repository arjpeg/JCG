...
lazy val jcg_soot_testadapter = project.settings(
    ...
    libraryDependencies += "org.soot-oss" % "soot" % "4.4.1",
    libraryDependencies += "com.typesafe.play" %% "play-json" % "2.9.2",
    // Force SBT to include all transitive dependencies
    excludeDependencies ++= Seq(
      // Don't exclude anything, this forces inclusion
    ),
    // Explicitly mark that we want all transitives
    transitiveClassifiers := Seq.empty,
    aggregate in assembly := false,
    publishArtifact := false
).dependsOn(jcg_testadapter_commons)
